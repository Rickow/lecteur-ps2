...
/* ps2mc-android : le cache de clusters est un état global. Comme on jongle entre
 * deux cartes ouvertes en même temps, il faut pouvoir vider ce cache dans le
 * buffer de la carte courante avant de basculer. Card_FlushMCCache() est static,
 * d'où ce wrapper. Voir patches/0001-mcio-flush.patch. */
int mcio_mcFlush(void)
{
	return Card_FlushMCCache();
}

/* ps2mc-android : formate une carte même vierge.
 *
 * Le mcio_mcFormat() d'origine appelle mcio_mcDetect() en préambule et abandonne
 * si la carte n'est pas déjà détectée — donc inutilisable sur une image vierge
 * (0xFF), qui n'a pas de superblock d'où tirer les specs. On sème ici un
 * superblock de carte 8 Mo standard quand il manque, on peuple les specs, puis on
 * laisse Card_Format() reconstruire FAT + répertoire racine (il gère le cas
 * sceMcResNoFormat). Voir patches/0002-mcio-format-blank.patch. */
int mcio_mcFormatBlank(void)
{
	struct MCDevInfo *mcdi = (struct MCDevInfo *)&mcio_devinfo;

	/* Carte déjà formatée : chemin standard (réécrit un superblock propre). */
	if (memcmp(SUPERBLOCK_MAGIC, vmc_data, 28) == 0) {
		int r = mcio_mcDetect();
		if (r != sceMcResSucceed && r != sceMcResNoFormat)
			return r;
	...
	/* Carte vierge : on ne peut pas tirer les specs d'un superblock absent, et
	 * en semer un dans vmc_data sans ECC valide fait échouer la détection. On
	 * peuple donc directement mcio_devinfo avec la géométrie d'une carte 8 Mo
	 * standard, et Card_Format() écrit lui-même un superblock avec un ECC
	 * correct. La géométrie (nombre de clusters, mode ECC) se déduit de la
	 * taille de l'image. */
	int noecc = (vmc_size % 0x800000 == 0);
	int page_stride = noecc ? 512 : 528;
	uint32_t clusters = (uint32_t)(vmc_size / page_stride) / 2;

	memset((void *)mcdi, 0, sizeof(struct MCDevInfo));
	memcpy(mcdi->magic, SUPERBLOCK_MAGIC, 28);
	memcpy(mcdi->version, SUPERBLOCK_VERSION, 12);
	append_le_uint16((uint8_t *)&mcdi->pagesize, 512);
	append_le_uint16((uint8_t *)&mcdi->pages_per_cluster, 2);
	append_le_uint16((uint8_t *)&mcdi->blocksize, 16);
	append_le_uint16((uint8_t *)&mcdi->unused, 0xff00);
	append_le_uint32((uint8_t *)&mcdi->clusters_per_card, clusters);
	append_le_uint32((uint8_t *)&mcdi->cluster_size, MCIO_CLUSTERSIZE);
	append_le_uint32((uint8_t *)&mcdi->FATentries_per_cluster, MCIO_CLUSTERFATENTRIES);
	append_le_uint32((uint8_t *)&mcdi->clusters_per_block, 8); /* blocksize / pages_per_cluster */
	mcdi->cardtype = sceMcTypePS2;
	mcdi->cardflags = noecc ? (0x2b & ~CF_USE_ECC) : 0x2b;
	append_le_uint32((uint8_t *)&mcdi->cardform, sceMcResNoFormat);
